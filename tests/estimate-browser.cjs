...
await p.locator('textarea[name=description]').fill('1 couch');assert.equal(await price(),'Estimated $150');
await p.locator('#edit-estimate').click();await p.locator('[name=notes]').fill('2 flights of stairs.');assert(await p.locator('#price-result').isHidden());assert.equal(await p.evaluate(()=>sessionStorage.getItem('pickupEstimate')),null);assert.equal(await price(),'Estimated $200');assert((await p.locator('#price-details').innerText()).includes('2 flights'));
await p.locator('[name=notes]').fill('No stairs.');await p.locator('textarea[name=description]').fill('1 sectional sofa');assert.equal(await price(),'Estimated $220');
await choose({access:'Upstairs',elevator:'no'});assert(await p.locator('[name=flights]').isEnabled());await p.locator('[name=flights]').fill('2');assert.equal(await price(),'Estimated $270');
await choose({elevator:'yes'});assert(await p.locator('[name=flights]').isDisabled());assert.equal(await price(),'Estimated $235');
await choose({access:'Driveway or curbside',itemType:'property',load:'home',bedrooms:'1',fullness:'typical'});await p.locator('textarea[name=description]').fill('Entire apartment cleanout');assert.equal(await price(),'Estimated $635');
await choose({bedrooms:'3'});assert.equal(await price(),'Estimated $815');
...
await choose({itemType:'chairs',load:'few'});assert(await p.locator('[name=material]').first().isDisabled());await p.locator('textarea[name=description]').fill('2 wooden chairs. The couch is staying.');await p.locator('[name=quantity]').fill('2');assert.equal(await price(),'Estimated $95');
...
await p.locator('textarea[name=description]').fill('<img src=x onerror=alert(1)> 2 wooden chairs');assert.equal(await price(),'Estimated $95');assert.equal(await p.locator('#price-details img').count(),0);
