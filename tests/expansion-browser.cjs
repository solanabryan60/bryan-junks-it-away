...
(async()=>{
 ...
 for(const [name,value] of Object.entries({area:'San Gabriel Valley',load:'few',itemType:'chairs',weight:'light',access:'Driveway or curbside',disassembly:'no',tight:'no',jobType:'home',timing:'This week',parking:'Yes, driveway or curbside'}))await page.locator(`input[name="${name}"][value="${value}"]`).check();
 await page.locator('[name="quantity"]').fill('2');await page.locator('button[type="submit"]').click();assert.equal(await page.locator('#price-value').innerText(),'$95');await page.locator('#reserve-estimate').click();await page.waitForFunction(()=>document.querySelector('[name="items"]')?.value.includes('chairs'));
