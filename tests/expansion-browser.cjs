...
(async()=>{
 ...
 await page.locator('[name="quantity"]').fill('2');await page.locator('textarea[name="description"]').fill('2 wooden chairs');await page.locator('button[type="submit"]').click();assert.equal(await page.locator('#price-value').innerText(),'Estimated $95–$120');await page.locator('#reserve-estimate').click();await page.waitForFunction(()=>document.querySelector('[name="items"]')?.value.includes('chairs'));
